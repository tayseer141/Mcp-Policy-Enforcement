...
def get_user_or_raise(db: Session, username: str) -> User:
    ...
    Ensures the user exists locally before forwarding the request to the
    MCP server (fast fail + clearer error than going over the wire).
    """
...
def run_tool_for_user(
    ...
    2. Inject username (identity) into the tool arguments.
    3. Forward the request to the real MCP server.
    4. The MCP server runs RBAC + policy enforcement and execution.

    Raises:
        ValueError:      unknown user (local check).
        PermissionError: MCP server denied the call (RBAC or policy).
